...
public class NetECPacketReceiver implements Closeable {
   ...
   */
  private NetECPacketHeader curHeader;
  ...
  private void doRead(ReadableByteChannel ch, InputStream in)
    ...
    //         HEADER                     DATA
    //  <HEADER_LENGTH Bytes>  <PACKET_SIZE - HEADER bytes>
    //
    //
    ...
    if (curHeader == null) {
      curHeader = new NetECPacketHeader();
    }
    curHeader.setFieldsFromData(headerBuf);
  ...
  private void reslicePacket() {
    ...
    //         HEADER                     DATA
    //  <HEADER_LENGTH Bytes>  <PACKET_SIZE - HEADER bytes>
    curPacketBuf.position(NetECPacketHeader.HEADER_LENGTH);
    curPacketBuf.limit(PACKET_SIZE);
